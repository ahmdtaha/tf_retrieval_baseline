...
def endpoints(image, is_training,weight_decay=0.0):
    if image.get_shape().ndims != 4:
        raise ValueError('Input must be of size [batch, height, width, 3]')

    image = image - tf.constant(_RGB_MEAN, dtype=tf.float32, shape=(1,1,1,3))

    with tf.contrib.slim.arg_scope(resnet_arg_scope(batch_norm_decay=0.9, weight_decay=weight_decay)):
    ...
    endpoints['attention'] = endpoints['resnet_v1_50/block4']
