...
def main(argv):
    ...
    init_iter = tf.data.Iterator.from_structure(dataset.output_types, dataset.output_shapes)
    ...
    images_ph = tf.placeholder(dataset.output_types[0], dataset.output_shapes[0])
    ...
    with h5py.File(args.filename, 'w') as f_out, tf.Session(config=gpu_config) as sess:
        ...
        for start_idx in count(step=args.batch_size):
            try:
                ...
                emb = sess.run(batch_embedding,feed_dict={images_ph:current_imgs})
